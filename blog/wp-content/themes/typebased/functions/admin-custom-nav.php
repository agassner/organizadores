		...
		foreach ($woo_custom_nav_menu as $woo_custom_nav_menu_items) {
			...
			if ($woo_custom_nav_menu_items->link_type == 'page')
			{
				...
				if ($woo_custom_nav_menu_items->custom_title == '') {
					$title = htmlentities(get_the_title($woo_custom_nav_menu_items->post_id));
				}
				else {
					$title = htmlentities($woo_custom_nav_menu_items->custom_title);
				}

				if ($woo_custom_nav_menu_items->custom_description == '') {
					$description = htmlentities(get_post_meta($woo_custom_nav_menu_items->post_id, 'page-description', true));
				}
				else {
					$description = htmlentities($woo_custom_nav_menu_items->custom_description);
				}
				$target = '';
			}
			//Category Menu Item
			elseif ($woo_custom_nav_menu_items->link_type == 'category') 
			{
				...
				if ($woo_custom_nav_menu_items->custom_description == '') {
					$description = htmlentities(strip_tags(category_description($woo_custom_nav_menu_items->post_id)));
				}
				else {
					$description = htmlentities($woo_custom_nav_menu_items->custom_description);
				}
			...
			if (isset($woo_custom_nav_menu_items->custom_anchor_title)) {
				$anchor_title = htmlentities($woo_custom_nav_menu_items->custom_anchor_title);
			}
			else {
				$anchor_title = $title;
			}
			
			if ($queried_id == $woo_custom_nav_menu_items->post_id) {
